...
class Utkarsh_v1_short_equal_lowerbound(Strategy):
    ...
    def to_sell(self, portfolio):
        ...
        for stock in portfolio.stocks:
            if (round(metric["metric"][stock.ticker], 3) <= 0) and (metric["three_wk_range"][stock.ticker] > 0.5):
...
class Utkarsh_v1_medium_equal_lowerbound(Strategy):
    def __init__(self, name):
        Strategy.__init__(self, name)
...
class Utkarsh_v1_long_equal_lowerbound(Strategy):
    def __init__(self, name):
        Strategy.__init__(self, name)
    ...
    def to_sell(self, portfolio):
        ...
        sell_list = []
        for stock in portfolio.stocks:
            if (round(metric["metric"][stock.ticker], 3) <= 0) and (metric["three_m_range"][stock.ticker] > 0.5):
                sell_list.append((stock.ticker, 1))
        return sell_list
